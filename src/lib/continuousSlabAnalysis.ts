...
export function analyzeContinuousStrip(
  ...
  // Find matching slab in slabs if available to retrieve properties
  const firstSpan = strip.spans[0];
  const matchingSlab = slabs?.find(s => s.id === firstSpan?.slabId);

  const thickness = matchingSlab?.thickness ?? slabProps.thickness;
  const finishLoad = matchingSlab?.finishLoad ?? slabProps.finishLoad;
  const liveLoad = matchingSlab?.liveLoad ?? slabProps.liveLoad;
  const cover = matchingSlab?.cover ?? slabProps.cover;
  const phiSlab = matchingSlab?.phiSlab ?? slabProps.phiSlab;
  const isRibbed = matchingSlab?.type === 'ribbed';

  // حساب الأحمال
  let ownWeight = (thickness / 1000) * mat.gamma; // kN/m²
  if (isRibbed) ownWeight *= 0.65; // Reduced dead load for ribbed slabs

  const wDL = ownWeight + finishLoad;
  const wLL = liveLoad;
  const Wu = 1.2 * wDL + 1.6 * wLL; // kN/m²
  const wuPerMeter = Wu * 1.0; // kN/m لشريحة عرض 1م

  // الحد الأدنى لتسليح البلاطات
  const d = thickness - cover - Math.max(10, phiSlab || 12) / 2;
  const shrinkageRatio = mat.fy >= 420 ? 0.0018 : 0.0020;
  const AsMin = shrinkageRatio * 1000 * thickness; // mm²/m
...
export function analyzeAllContinuousSlabs(
  ...
  return strips.map(strip => analyzeContinuousStrip(strip, slabProps, mat, slabs));
}
