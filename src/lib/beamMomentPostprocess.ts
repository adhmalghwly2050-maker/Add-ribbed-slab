import type { Column, Beam, FrameResult } from '@/lib/structuralEngine';
...
export function postprocessFrameResultsForColumnFaces(
  ...
  columns: Column[],
  beamsWithLoads: Beam[],
  effectiveFrameEndReleases: Record<string, any>
): FrameResult[] {
  return raw.map((frameRes) => {
    return {
      ...frameRes,
      beams: frameRes.beams.map((bRes) => {
        const matchedBeam = beamsWithLoads.find((b) => b.id === bRes.beamId);
        if (!matchedBeam) return bRes;

        const isHoriz = matchedBeam.direction === 'horizontal';
        ...
        const posKey = `${matchedBeam.x1.toFixed(3)}_${matchedBeam.y1.toFixed(3)}_${matchedBeam.x2.toFixed(3)}_${matchedBeam.y2.toFixed(3)}`;
        const posKeyRev = `${matchedBeam.x2.toFixed(3)}_${matchedBeam.y2.toFixed(3)}_${matchedBeam.x1.toFixed(3)}_${matchedBeam.y1.toFixed(3)}`;
        const rel = effectiveFrameEndReleases[posKey] || effectiveFrameEndReleases[posKeyRev];

        let releaseLeft = false;
        let releaseRight = false;
        if (rel) {
          const isReversed = !!effectiveFrameEndReleases[posKeyRev] && !effectiveFrameEndReleases[posKey];
          if (isReversed) {
            releaseLeft = !!rel.nodeJ?.releaseMoment;
            releaseRight = !!rel.nodeI?.releaseMoment;
          } else {
            releaseLeft = !!rel.nodeI?.releaseMoment;
            releaseRight = !!rel.nodeJ?.releaseMoment;
          }
        }

        const sampleResult = sampleBeamEndMomentsAtPhysicalFaces({
          span: bRes.span,
          stations: bRes.momentStations,
          momentLeft: bRes.Mleft,
          momentRight: bRes.Mright,
          halfColLeft,
          halfColRight,
          releaseLeft,
          releaseRight,
        ...
        return {
          ...bRes,
          Mleft: sampleResult.Mleft,
          Mright: sampleResult.Mright,
          momentStations: sampleResult.stations,
        };
      }),
    };
  });
